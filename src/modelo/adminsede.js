...
export class AdminSede {
  ...
  static async traerPorIngresar(numeroIdentificacion, password) {
    const query =
      "SELECT * FROM adminsede WHERE numeroIdentificacion = ? AND password = ?";
    return new Promise((resolve, reject) => {
      connection.query(
        query,
        [numeroIdentificacion, password],
        (err, response) => {
          if (err) console.log(err);
          resolve(response);
        }
  ...
  static async agregar(adminSede) {
    const { numeroIdentificacion, nombre, apellidos, password, nombreMunicipio, nombreDepartamento } =
      adminSede;
    const query =
      "INSERT INTO adminsede(numeroIdentificacion, nombre, apellidos, password, nombreMunicipio, nombreDepartamento) VALUE (?, ?, ?, ?, ?, ?)";
    return new Promise((resolve, reject) => {
      connection.query(
        query,
        [numeroIdentificacion, nombre, apellidos, password,  nombreMunicipio, nombreDepartamento],
        (err, response) => {
          if (err) console.log("Error: ", err);
          resolve(response);
        }
  ...
  static async actualizar(adminSede, id) {
    const { numeroIdentificacion, nombre, apellidos, password, nombreMunicipio, nombreDepartamento} =
      adminSede;
    const query =
      "UPDATE adminsede SET numeroIdentificacion = ?, nombre = ?, apellidos = ?, password = ?, nombreMunicipio = ?, nombreDepartamento = ? WHERE numeroIdentificacion = ?";
    connection.query(
      query,
      [numeroIdentificacion, nombre, apellidos, password, nombreMunicipio, nombreDepartamento, id],
      (err, response) => {
        if (err) console.log("Error: ", err);
        return response;
      }
